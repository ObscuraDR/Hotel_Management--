...
const revenueData = [
  { month: "T10", revenue: 180 }, { month: "T11", revenue: 220 },
  { month: "T12", revenue: 310 }, { month: "T1",  revenue: 195 },
  { month: "T2",  revenue: 240 }, { month: "T3",  revenue: 248 },
];
...
const columns = [
    ...
    render: (v, _, i) => (
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: "#94a3b8" }}>Khách lẻ</div>
        </div>
      </div>
    ),
  },
  { title: "Phòng", dataIndex: "room_number", render: (v) => <span style={{ fontWeight: 500, color: "#475569", fontSize: 13 }}>{v}</span> },
  ...
  { title: "Số tiền", dataIndex: "amount", render: (v) => <span style={{ fontWeight: 700, color: "#10b981", fontSize: 13 }}>{Number(v).toLocaleString("vi-VN")}₫</span> },
];
...
export default function Dashboard() {
  ...
  const stats = [
    { title: "Tổng Phòng", value: dashData?.totalRooms ?? "—", icon: <HomeOutlined />, color: "#6366f1", gradient: "linear-gradient(135deg, #6366f1, #818cf8)", suffix: "phòng", change: "+5%", up: true, desc: "so với tháng trước" },
    { title: "Đang Có Khách", value: dashData?.occupiedRooms ?? "—", icon: <CalendarOutlined />, color: "#f59e0b", gradient: "linear-gradient(135deg, #f59e0b, #fbbf24)", suffix: "phòng", change: "+12%", up: true, desc: "so với hôm qua" },
    { title: "Khách Hôm Nay", value: dashData?.todayGuests ?? "—", icon: <UserOutlined />, color: "#10b981", gradient: "linear-gradient(135deg, #10b981, #34d399)", suffix: "khách", change: "+8%", up: true, desc: "so với hôm qua" },
    { title: "Doanh Thu Tháng", value: dashData?.monthRevenue ? (dashData.monthRevenue / 1000000).toFixed(1) + "tr" : "—", icon: <DollarOutlined />, color: "#ef4444", gradient: "linear-gradient(135deg, #ef4444, #f87171)", prefix: "₫", change: "-3%", up: false, desc: "so với tháng trước" },
  ];
    ...
    <div style={{ padding: 4 }}>
      {error && (
        <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: '12px 20px', marginBottom: 20, color: '#dc2626', fontSize: 13, display: 'flex', alignItems: 'center', gap: 8 }}>
          ⚠️ {error} — <button onClick={() => { setError(null); setLoading(true); api.dashboard().then(setDashData).catch(e => setError(e.message)).finally(() => setLoading(false)); }} style={{ color: '#6366f1', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Thử lại</button>
        </div>
      )}
        {/* Page Header */}
        <div style={{ background: "linear-gradient(135deg, #0f172a 0%, #1e3a5f 100%)", borderRadius: 20, padding: "28px 32px", marginBottom: 24, position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", position: "relative" }}>
            <div>
              ...
              <h1 style={{ color: "#fff", fontSize: 26, fontWeight: 800, margin: 0, lineHeight: 1.3 }}>Tổng Quan Hệ Thống</h1>
              <p style={{ color: "rgba(255,255,255,0.5)", margin: "6px 0 0", fontSize: 14 }}>
                {new Date().toLocaleDateString("vi-VN", { weekday: "long", year: "numeric", month: "long", day: "numeric" })} — Chào mừng trở lại! 👋
              </p>
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <Button style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", borderRadius: 10, height: 38 }}>Xuất báo cáo</Button>
              <Button style={{ background: "linear-gradient(135deg, #f59e0b, #d97706)", border: "none", color: "#fff", borderRadius: 10, height: 38, fontWeight: 600 }}>+ Đặt phòng mới</Button>
            </div>
          </div>
        </div>
        ...
        <Row gutter={[16, 16]} style={{ marginBottom: 20 }}>
          ...
          <Col xs={24} lg={15}>
            <Card variant="borderless" style={cardStyle}
              title={
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 15, color: "#1e293b" }}>Doanh Thu 6 Tháng Gần Nhất</div>
                    <div style={{ fontSize: 12, color: "#94a3b8", fontWeight: 400 }}>Đơn vị: triệu đồng</div>
                  </div>
                  <Button type="text" size="small" style={{ color: "#6366f1", fontWeight: 600, fontSize: 12 }}>Xem chi tiết <RightOutlined /></Button>
                </div>
              }
            ...
            >
              <div style={{ display: "flex", alignItems: "flex-end", gap: 12, height: 160, padding: "8px 0" }}>
                {revenueData.map((d, i) => (
                  <div key={d.month} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: i === 5 ? "#6366f1" : "#94a3b8" }}>{d.revenue}tr</div>
              ...
              <div style={{ marginTop: 16, padding: "12px 16px", background: "linear-gradient(135deg, #eef2ff, #f5f3ff)", borderRadius: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <span style={{ color: "#6366f1", fontWeight: 800, fontSize: 20 }}>{dashData?.monthRevenue ? (dashData.monthRevenue / 1000000).toFixed(1) + "tr" : "248tr"}</span>
                  <span style={{ color: "#94a3b8", fontSize: 13, marginLeft: 6 }}>tháng này</span>
                </div>
                <span style={{ background: "#ecfdf5", color: "#10b981", padding: "4px 12px", borderRadius: 20, fontSize: 12, fontWeight: 700 }}>
                  <ArrowUpOutlined /> +12.5% so với T2
                </span>
              </div>
            </Card>
          </Col>
          ...
          {/* Room Status */}
          <Col xs={24} lg={9}>
            <Card variant="borderless" style={{ ...cardStyle, height: "100%" }} title="Tình Trạng Phòng" styles={{ header: cardHeadStyle }}>
              <div style={{ textAlign: "center", padding: "16px 0 20px", background: "linear-gradient(135deg, #fffbeb, #fef3c7)", borderRadius: 14, marginBottom: 20 }}>
                ...
                <div style={{ color: "#92400e", fontSize: 13, fontWeight: 600, marginTop: 4 }}>Tỷ lệ lấp đầy hôm nay</div>
                <div style={{ color: "#b45309", fontSize: 12, marginTop: 2 }}>
                  {dashData ? `${occupiedRooms} / ${totalRooms} phòng đang có khách` : 'Đang tải...'}
                </div>
              </div>
                  ...
                  <div key={r.type}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                      ...
                      <span style={{ fontSize: 12, color: "#94a3b8", fontWeight: 500 }}>
                        {r.occupied}/{r.total}
                        <span style={{ color: roomTypeColors[r.type] || "#6366f1", fontWeight: 700, marginLeft: 4 }}>({Math.round((r.occupied / r.total) * 100)}%)</span>
                      </span>
                    </div>
                    <Progress percent={Math.round((r.occupied / r.total) * 100)} strokeColor={roomTypeColors[r.type] || "#6366f1"} trailColor="#f1f5f9" size="small" showInfo={false} strokeLinecap="round" />
                  </div>
                )) : (
                <div style={{ textAlign: 'center', padding: 20 }}>
                  <span style={{ color: '#94a3b8', fontSize: 13 }}>Chưa có dữ liệu phòng</span>
                </div>
              )}
              </div>
            </Card>
        ...
        <Card variant="borderless" style={cardStyle}
          title={
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontWeight: 700, fontSize: 15, color: "#1e293b" }}>Đặt Phòng Gần Đây</div>
              <Button type="text" size="small" style={{ color: "#6366f1", fontWeight: 600, fontSize: 12 }}>Xem tất cả <RightOutlined /></Button>
            </div>
          }
          styles={{ header: cardHeadStyle }}
        >
