...
const rewards = [
  { id: 1, name: "100,000₫ off next booking", points: 200, icon: "🎟️" },
  { id: 2, name: "Free breakfast (2 people)", points: 350, icon: "☕" },
  { id: 3, name: "Free room upgrade", points: 500, icon: "⬆️" },
  { id: 4, name: "Free 60-min spa", points: 800, icon: "💆" },
  { id: 5, name: "1 free night (Standard)", points: 1500, icon: "🏨" },
];
...
export default function GuestProfile() {
  ...
  if (!guest) {
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <Button type="primary" onClick={() => navigate("/guest/login")} style={{ background: "#6366f1", borderRadius: 10 }}>Sign In</Button>
      </div>
    );
  }
  ...
  const tabItems = [
          ...
          {!editMode ? (
            <div className="grid grid-cols-2 gap-4" style={{ paddingTop: 16 }}>
              {[
                ...
                { label: "Date of Birth", value: guest.dob || "—" },
                { label: "ID/Passport", value: guest.idNumber || "—" },
                { label: "Join Date", value: guest.joinDate },
                { label: "Membership Tier", value: guest.tier },
              ].map((item) => (
              ...
              <div style={{ gridColumn: "1/-1", display: "flex", gap: 8, marginTop: 4 }}>
                <Button icon={<EditOutlined />} type="primary" onClick={() => { setEditMode(true); form.setFieldsValue(guest); }} style={{ borderRadius: 10, background: "#6366f1" }}>Edit</Button>
                <Button icon={<LockOutlined />} onClick={() => setPwdModal(true)} style={{ borderRadius: 10 }}>Change Password</Button>
              </div>
            </div>
          ) : (
            <Form form={form} layout="vertical" onFinish={saveProfile} style={{ paddingTop: 16 }}>
              ...
              <Row gutter={12}>
                <Col span={12}><Form.Item name="dob" label="Date of Birth"><Input placeholder="dd/mm/yyyy" style={{ borderRadius: 8 }} /></Form.Item></Col>
                <Col span={12}>
                  <Form.Item name="nationality" label="Nationality">
                    <Select style={{ borderRadius: 8 }}>
                      {["Vietnam", "USA", "Japan", "South Korea", "Other"].map((n) => <Option key={n} value={n}>{n}</Option>)}
                    </Select>
                  </Form.Item>
                </Col>
              </Row>
              <Form.Item name="idNumber" label="ID/Passport"><Input style={{ borderRadius: 8 }} /></Form.Item>
              <div className="flex gap-2">
                <Button type="primary" htmlType="submit" style={{ background: "#6366f1", borderRadius: 8 }}>Save</Button>
    ...
    {
      key: "redeem",
      label: <span><GiftOutlined /> Redeem Points</span>,
      children: (
        <div style={{ padding: "16px 0" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, padding: "12px 16px", background: "#fffbeb", borderRadius: 12 }}>
            <span style={{ fontWeight: 600, color: "#92400e" }}>⭐ Your current points</span>
              ...
              <div key={r.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", borderRadius: 12, background: guest.points >= r.points ? "#f8fafc" : "#fafafa", border: `1px solid ${guest.points >= r.points ? "#e2e8f0" : "#f1f5f9"}`, opacity: guest.points >= r.points ? 1 : 0.5 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 12, color: "#f59e0b", margin: "2px 0 0", fontWeight: 700 }}>{r.points} pts</p>
                  </div>
                </div>
                <Button type="primary" size="small" disabled={guest.points < r.points}
                  onClick={() => {
                    const result = redeemPoints(guest.id, r.points, r.name);
                    if (result.success) { setGuest(result.guest); message.success(`Redeemed: ${r.name}!`); }
          ...
          {guest.redeemHistory?.length > 0 && (
            <div style={{ marginTop: 20 }}>
              <p style={{ fontWeight: 700, color: "#475569", marginBottom: 10 }}><HistoryOutlined /> Redemption History</p>
    ...
    },
    {
      ...
      children: (
        <div style={{ padding: "16px 0" }}>
          {bookings.length === 0 ? (
            <p style={{ textAlign: "center", color: "#94a3b8", padding: 24 }}>No transactions yet</p>
          ) : (
            bookings.map((b) => (
              <div key={b.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", background: "#f8fafc", borderRadius: 12, marginBottom: 8 }}>
                ...
                <div style={{ textAlign: "right" }}>
                  ...
                  <Tag color={b.status === "Completed" ? "default" : b.status === "Upcoming" ? "blue" : "red"} style={{ fontSize: 11, margin: 0 }}>{b.status}</Tag>
                </div>
              </div>
            ))
          )}
    ...
    {
      key: "settings",
      label: <span><BellOutlined /> Settings</span>,
      children: (
        <div style={{ padding: "16px 0", display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ padding: "16px", background: "#f8fafc", borderRadius: 12 }}>
            <p style={{ fontWeight: 700, color: "#1e293b", margin: "0 0 12px", display: "flex", alignItems: "center", gap: 6 }}><GlobalOutlined /> Language</p>
          ...
          <div style={{ padding: "16px", background: "#f8fafc", borderRadius: 12 }}>
            <p style={{ fontWeight: 700, color: "#1e293b", margin: "0 0 12px", display: "flex", alignItems: "center", gap: 6 }}><BellOutlined /> Notifications</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {[
                { key: "email", label: "Email Notifications", desc: "Receive booking confirmations via email" },
                { key: "sms", label: "SMS Notifications", desc: "Receive check-in reminders via SMS" },
                { key: "promo", label: "Deals & Promotions", desc: "Get the latest deals and offers" },
              ].map((n) => (
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      <div>
        ...
        <p className="text-gray-400 text-sm">Manage your personal info and account</p>
      </div>
      ...
      <Row gutter={[20, 20]}>
                ...
                {tier.next && (
                  <>
                    ...
                    <p className="text-xs text-gray-400 text-center">
                      Need <strong>{tier.pointsNeeded - guest.points} pts</strong> more to reach <strong>{tier.next}</strong>
                    </p>
                  </>
                )}
                {!tier.next && <p className="text-xs text-center font-semibold" style={{ color: tier.color }}>🎉 You are at the highest tier!</p>}
              </div>
            </Card>

            {/* Tier benefits */}
            <Card bordered={false} style={{ borderRadius: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}
              title={<span className="font-semibold">Membership Benefits</span>}>
              <div className="space-y-3">
                {[
                  { tier: "Bronze", color: "#cd7f32", benefits: ["Online booking", "Basic points earning"] },
                  { tier: "Silver", color: "#9ca3af", benefits: ["5% off all rooms", "Early check-in"] },
                  { tier: "Gold", color: "#f59e0b", benefits: ["10% off all rooms", "Free room upgrade"] },
                  { tier: "Platinum", color: "#6366f1", benefits: ["20% off all rooms", "Personal butler, Late check-out"] },
                ].map((t) => (
        ...
        <Col xs={24} lg={16}>
          ...
          <Card bordered={false} style={{ borderRadius: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", marginTop: 16 }}
            title={<span className="font-semibold">My Statistics</span>}>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                { label: "Total Bookings", value: bookings.length, color: "#6366f1", bg: "#eef2ff", icon: "📅" },
                { label: "Completed", value: bookings.filter((b) => b.status === "Completed").length, color: "#10b981", bg: "#ecfdf5", icon: "✅" },
                { label: "Upcoming", value: bookings.filter((b) => b.status === "Upcoming").length, color: "#f59e0b", bg: "#fffbeb", icon: "⏳" },
                { label: "Points", value: `${guest.points}pts`, color: tier.color, bg: tier.bg, icon: "⭐" },
              ].map((s) => (
      ...
      <Modal title="Change Password" open={pwdModal} onCancel={() => { setPwdModal(false); pwdForm.resetFields(); }}
        onOk={() => pwdForm.submit()} okText="Confirm" cancelText="Cancel" okButtonProps={{ style: { background: "#6366f1" } }}>
